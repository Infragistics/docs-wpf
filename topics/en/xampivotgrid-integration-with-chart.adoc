...
[NOTE]
====
It is recommended to use the `Zoomable` property to enable the zoombars on the DataChart, because the data may be too large to be displayed in the view, so the zoombars will allow scrolling and zooming on the desired section of the data.
====
