...
[start=2]
. Create your own custom class that derives from the  pick:[sl,win-rt=" link:{RootAssembly}{ApiVersion}~infragistics.summaryoperandbase.html[SummaryOperandBase]"]  pick:[wpf=" link:{ApiPlatform}datamanager{ApiVersion}~infragistics.summaryoperandbase.html[SummaryOperandBase]"]  class and name it StandardDeviationOperand.
