...
[start=3]
. Include a data source of one-dimensional numeric values. *Note:* With the Win/Loss type of Sparkline, only two values will be used for the chart – the highest one and the lowest one. The rest of the values in the data set will be ignored.
