...
The link:{ApiPlatform}datapresenter{ApiVersion}~infragistics.windows.datapresenter.xamtreegrid.html[xamTreeGrid] control allows you to configure (via property) which records are selected in response to a user selecting a range of records or cells. The three cases below have the same initial state:
