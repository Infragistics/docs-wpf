...
First, you should avoid directly accessing the content control attached to a cell since the content control might not exist. As mentioned previously, controls are only created when in view; therefore, you should only access a cell's attached content control when you know it is in the view port. The xamGrid control exposes the link:{ApiPlatform}controls.grids.xamgrid{ApiVersion}~infragistics.controls.grids.xamgrid~cellcontrolattached_ev.html[CellControlAttached] event to let you know when a control has been attached to a cell.
