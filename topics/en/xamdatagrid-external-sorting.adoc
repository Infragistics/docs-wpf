...
[NOTE]
====
The external sorting process does not support sorting of fields defined with binding type _Unbound_ or _UseAlternateBinding_. Taken with current limitations, the external sorting process only applies to the root field’s layout and not the child field’s layout. This is because there are multiple collection views for each child field layout.
====

In order to use the external sorting you need to set the `FieldSettings`{empty}'s link:{ApiPlatform}datapresenter{ApiVersion}~infragistics.windows.datapresenter.fieldlayoutsettings~sortevaluationmodeproperty.html[SortEvaluationMode] property to either _Manual_ or _UseCollectionView_, depending on your data source type. The available options of the enumerations are:
...
| _Default_ 
|This option defaults to _Auto_.
