...
If you want to apply a theme to an application, a page or a control, you may choose from the provided five theme packages. The theme packages contain resource dictionaries for all available {ProductName} controls as well as themes for the supported Microsoft WPF controls.

=== Requirements summary chart

The following table summarizes the available theme packages and required NuGet package to use that theme. For more information on setting up the NuGet feed and adding NuGet packages, you can take a look at the following documentation: link:nuget-feeds.html[NuGet Feeds].
