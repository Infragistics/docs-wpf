...
* <<Ref_26_1_21, 26.1.21 (2026 年 6 月)>>
* <<Ref_26_1, 26.1 (2026 年リリース)>>
...
|`3065`
|DataChart
|ScatterLineSeries とカスタムラベルコンバーターを使用した XamDataChart における回帰問題で、添字が範囲外になるエラーが発生する問題を修正
...
- `MarkerSeries` に `MarkerSize` をを追加しました。これにより、サポートされているチャートシリーズのマーカーに対してデバイス非依存ピクセルで正確な幅と高さを設定し、`NaN` を指定してテンプレートのサイズ設定を復元できるようになります。
...
|`2281`
|DonutChart
|テーマ適用時に DonutChart で LegendItemBadgeTemplate が表示されない。

|`3075`
|TileManager
|垂直レイアウト順を使用している場合、最後のタイルのみが表示されている状態でそのタイルが削除されると、XamTileManager にタイルが表示されなくなる。

|`3104`
|DataPresenter
|子レコードの上に表示されるヘッダーのインデントが、ソース レコードと同じ値になっていない。

|`3284`
|DataTree
|XamDataTree: ノードがインライン編集モードのとき、外部ボタンのクリック/コマンドが発火しない。

|`33893`
|TextEditor
|編集モードに入るとき TextEditor がわずかにリサイズされる。

|`34441`
|PropertyGrid
|EditTemplate で ContentPresenter を使用している場合、プロパティ グリッドをスクロールすると値が変わる。

|`39084`
|TextEditor
|読み込んだ RTF ファイルに対して、XamRichTextEditor が水平スクロールバーを誤って表示する。

|`39370`
|DataGrid
|CalculationSettings の数式が設定されたアンバインド フィールドで DataGrid をグループ化しても再計算されない。

|`40128`
|TreeGrid
|XamDataTree を含むアプリケーションで 'System.InvalidOperationException: Collection was modified' が発生する。

|`41419`
|Excel
|VBA 署名付き Excel ファイルを保存しても署名/証明書が保持されない。
...
|`2391`
|XamDataGrid
|RecordSelectorNumberType を DataItemIndex に設定したグリッドで、水平スクロールバーが表示される。

|`2864`
|XamRichTextEditor
|XamRichTextEditor - すべてを選択して外部ソースからコンテンツを貼り付けると例外が発生する。

|`3075`
|XamTileManager
|XamTileManager - 垂直レイアウト順序を使用している場合、最後のタイルのみが表示されている状態でそのタイルが削除されると、タイルが表示されなくなる。

|`3104`
|XamDataPresenter
|子レコードの上に表示されるヘッダーが、ソース レコードと同じ値にインデントされていない。

|`31788`
|XamDataPresenter
|XamDataGrid - ChildRecordsDisplayOrder "BeforeParent" のプレビュー。

|`38668`
|DataTooltipLayer
|チャートの TitleTextColor が使用されると、TitleTextColor がオーバーライドされる。
...
|`40238`
|Excel
|Excel 数式パーサーの修正 - Workbook.Load() が Excel.FormulaParseException をスローする。

|`40447`
|XamDataGrid
|MergedCells - 結合されたセルが期待どおりに印刷されない。

|`41431`
|XamDataTree
|XamDataTreeNodeControl を再テンプレートしてノードを選択すると ArgumentNullException が発生する。

|`41167`
|Excel
|オブジェクトの数式がラウンドトリップされない - カメラ ツールのラウンドトリップの Excel サポートを追加。
...
|`41457`
|XamRichTextEditor
|RichTextEditor - テーブルを含む .rtf ファイルの読み込み時に、テーブルの書式設定なしで読み込まれることがある。

|`41594`
|XamDataChart
|AssigningCategoryStyle の args.GetItems が null であるか、フラグメント シリーズの項目を更新するために機能しない。
