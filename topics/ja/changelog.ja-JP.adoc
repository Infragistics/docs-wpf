...
|`2281`
|DonutChart
|テーマ適用時に DonutChart の LegendItemBadgeTemplate が表示されない。

|`3075`
|TileManager
|垂直レイアウト順序を使用している場合、最後のタイルのみが表示されている状態でそのタイルが削除されると、XamTileManager にタイルが表示されなくなる。
...
|`39084`
|TextEditor
|XamRichTextEditor が読み込んだ RTF ファイルに対して水平スクロールバーを誤って表示する。
